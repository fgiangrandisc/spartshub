...
const entradaPublica = () => {
  const path = (window.location.pathname || "/").replace(/\/+$/, "") || "/";
  return path === "/buscar" || path === "/servicios" || path === "/arriendos" || path.startsWith("/categoria/");
};
...
/* Igual que fmtPrice, pero para servicios y arriendos: agrega el sufijo de
   la modalidad de tarifa (/hora, /visita, /día, /semana, /mes) cuando
   corresponde. Mismo campo `rate_type`, distintas modalidades según kind. */
...
const fmtRate = (p, cur, rateType) => {
  ...
  const suffix = RATE_SUFFIXES[rateType];
...
/* ══════════════════════════════════════════════════════════════
   LANDING PAGE
══════════════════════════════════════════════════════════════ */
function LandingPage({ onLogin, onRegister, onSearch, onEnter, onGateRegister, onEnterServices, onEnterRentals }) {
  ...
  const CENTER_NAV = [
    ...
    { key:"nav_rentals",  action:onEnterRentals||onEnter },
  ];
...
function RentalsPage({ user, onSelect, region, onPublish }) {
  const [q,         setQ]         = useState("");
  const [cat,       setCat]       = useState("all");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    let query = sb.from("listings").select("*").eq("kind","arriendo").order("created_at",{ascending:false});
    if (cat !== "all") query = query.eq("cat", cat);
    if (rentalType !== "all") query = query.eq("rental_type", rentalType);
    if (q && q.trim()) {
    ...
    if (q && q.trim()) {
      resultados = resultados.filter(l =>
        coincideBusqueda([l.title, l.description].filter(Boolean).join(" "), q)
      );
    }
    setListings(resultados);
    setLoading(false);
  }, [cat, rentalType, q, ubicacion, region]);
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:10, marginBottom:12, alignItems:"center", flexWrap:"wrap" }}>
        <div className="search-bar" style={{ flex:"1 1 240px", minWidth:0 }}>
          <Ic n="search" s={16} c={MUTED}/>
          <input placeholder="Buscar un arriendo…" value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&load()}/>
          {q && <button className="btn-ghost" style={{ padding:"2px 4px" }} onClick={()=>setQ("")}><Ic n="x" s={16} c={MUTED}/></button>}
        </div>
        {user && (
          <button className="btn-red" onClick={onPublish} style={{ padding:"10px 18px", fontSize:15, whiteSpace:"nowrap" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
        <select className="inp" value={rentalType} onChange={e=>setRentalType(e.target.value)} style={{ width:"auto", minWidth:160 }}>
          <option value="all">Todos los tipos</option>
          {Object.entries(RENTAL_TYPE_LABELS).map(([id,label])=><option key={id} value={id}>{label}</option>)}
        </select>
        <select className="inp" value={cat} onChange={e=>setCat(e.target.value)} style={{ width:"auto", minWidth:160 }}>
          {CATS.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
      ...
      <p style={{ fontSize:16, color:MUTED, marginBottom:14 }}>
        {loading ? "Buscando…" : `${listings.length} ${listings.length===1?"arriendo":"arriendos"}`}
      </p>
      ...
      {loading ? (
        ...
        <div style={{ padding:"60px 0", textAlign:"center" }}>
          <div style={{ fontSize:48, marginBottom:12 }}>🔑</div>
          <p className="bebas" style={{ fontSize:24, color:TEXT, marginBottom:8 }}>Todavía no hay arriendos acá</p>
          <p style={{ color:MUTED, fontSize:16, marginBottom:16 }}>
            {user ? "Sé el primero en publicar un arriendo en esta categoría o región." : "Iniciá sesión para publicar tu arriendo."}
          </p>
          {user && <button className="btn-ol" onClick={onPublish} style={{ fontSize:16 }}>+ Publicar arriendo</button>}
        </div>
      ) : (
        <div style={{ display:"grid", gridTemplateColumns:`repeat(auto-fill, minmax(${isMobile?150:230}px, 1fr))`, gap:isMobile?12:16 }}>
          {listings.map(l=>(
            <div key={l.id} className="photo-card card" onClick={()=>onSelect(l)}>
              <PhotoPlaceholder emoji={l.emoji||"🔑"} url={l.photos?.[0]} h={130} alt={l.title}/>
              <div style={{ padding:"10px 12px 14px" }}>
                <span className="tag t-dim" style={{ fontSize:16, marginBottom:6, display:"inline-block" }}>{RENTAL_TYPE_LABELS[l.rental_type]||CATS.find(c=>c.id===l.cat)?.label||"—"}</span>
                <p style={{ fontSize:17, fontWeight:700, lineHeight:1.3, marginBottom:4, color:TEXT }}>{l.title}</p>
                <p style={{ fontSize:15, color:SUB, marginBottom:2 }}>{l.biz}</p>
                {l.location && <p style={{ fontSize:15, color:MUTED, marginBottom:5 }}>📍 {l.location}</p>}
                <p className="bebas" style={{ fontSize:19, color:RED, marginTop:2 }}>{fmtRate(l.price, l.currency, l.rate_type)}</p>
        ...
        <div style={{ overflowY:"auto",flex:1,paddingBottom:40 }}>
          ...
          <div style={{ padding:"20px 20px 0" }}>
            ...
            <p className="bebas" style={{ fontSize:30,color:RED,marginBottom:16 }}>
              {(l.kind==="servicio"||l.kind==="arriendo") ? fmtRate(l.price, l.currency, l.rate_type) : fmtPrice(l.price, l.currency)}
            </p>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20 }}>
              {l.kind==="servicio" ? [
                ...
                l.rental_type&&["Tipo de arriendo",RENTAL_TYPE_LABELS[l.rental_type]||l.rental_type],
                (l.rental_type==="maquinaria"||l.rental_type==="vehiculo")&&["Operador/conductor",l.includes_operator?"Incluido":"No incluido"],
                l.min_period&&["Período mínimo",l.min_period],
                l.deposit&&["Depósito de garantía",l.deposit],
              ].filter(Boolean).map(([k,v])=>(
                <div key={k} style={{ background:BG2,borderRadius:10,padding:"12px 14px",border:`1px solid ${BORDER}` }}>
                  <p style={{ fontSize:16,color:MUTED,marginBottom:3,fontWeight:600,textTransform:"uppercase",letterSpacing:.5 }}>{k}</p>
                  <p style={{ fontSize:16,fontWeight:600,color:TEXT }}>{v}</p>
...
function PublishSheet({ user, profile, onClose, onDone, onBulkUpload, initialType }) {
  ...
  const [f, setF] = useState({
    ...
    // Solo para arriendos (formulario distinto al de productos y servicios):
  ...
  const submit = async () => {
    ...
    if (type === "arriendo") return submitRental();
  ...
  // Publicar un ARRIENDO: maquinaria/equipo, bodega/espacio o vehículo con
  // conductor. Formulario y payload propios (tipo de arriendo, operador
  // incluido, depósito, período mínimo) — no corre el motor de matching.
  const submitRental = async () => {
    const isNeg = f.rate_type === "convenir";
    if (!f.title || (!isNeg && !f.price)) { setErr(t("pub_error_required")); return; }
    setLoading(true); setErr("");
    const { data:inserted, error } = await sb.from("listings").insert({
      user_id:user.id, kind:"arriendo", title:f.title,
      cat:f.cat, operation:"Arriendo",
  ...
  const TYPES = [
    ...
    { id:"arriendo", icon:"box",     titleKey:"pub_rental",   subKey:"pub_rental_sub" },
      ...
      <div className="sheet sheet-up" style={{ maxHeight:"94dvh",overflow:"hidden",display:"flex",flexDirection:"column",...sheetStyle }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"8px 20px 16px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            ...
            <h3 className="bebas" style={{ fontSize:22,color:TEXT }}>{step===0?"Nueva publicación":type==="producto"?"Publicar producto":type==="servicio"?"Publicar servicio":type==="arriendo"?"Publicar arriendo":"Publicar"}</h3>
          </div>
          <button className="btn-ghost" style={{ padding:"6px" }} onClick={onClose}><Ic n="x" s={20} c={MUTED}/></button>
        </div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
              ...
              {previews.length === 0 ? (
                <div onClick={()=>photoInputRef.current?.click()}
                  ...
                  <Ic n="camera" s={36} c={RED}/>
                  <p style={{ fontSize:17, fontWeight:700, color:RED, fontFamily:"Barlow Condensed,sans-serif", letterSpacing:.5, textTransform:"uppercase", margin:0 }}>Agregar fotos</p>
                  <p style={{ fontSize:14, color:MUTED, margin:0, textAlign:"center" }}>Equipo, espacio o vehículo · Hasta 4 fotos</p>
                </div>
              ) : (
                <div>
                  <div style={{ display:"flex", gap:10, overflowX:"auto", paddingBottom:6 }}>
              ...
              <div>
                <p style={{ fontSize:16,fontWeight:700,color:MUTED,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>Título *</p>
                <input className="inp" placeholder="Ej: Excavadora Komatsu PC200 en arriendo" value={f.title} maxLength={200} onChange={e=>upd("title",e.target.value)}/>
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
                <div>
                  <p style={{ fontSize:16,fontWeight:700,color:MUTED,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>Período mínimo <span style={{ fontWeight:400,textTransform:"none" }}>{t("optional")}</span></p>
                  <input className="inp" placeholder="Ej: 3 días" value={f.min_period} maxLength={60} onChange={e=>upd("min_period",e.target.value)}/>
                </div>
                <div>
                  <p style={{ fontSize:16,fontWeight:700,color:MUTED,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>Depósito de garantía <span style={{ fontWeight:400,textTransform:"none" }}>{t("optional")}</span></p>
              ...
              <div>
                <p style={{ fontSize:16,fontWeight:700,color:MUTED,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>Descripción</p>
                <textarea className="inp" rows={3} placeholder="Condiciones del arriendo, estado del equipo/espacio, etc." value={f.description} maxLength={1000} onChange={e=>upd("description",e.target.value)} style={{ resize:"none" }}/>
              </div>

              <div>
                <p style={{ fontSize:16,fontWeight:700,color:MUTED,marginBottom:6,textTransform:"uppercase",letterSpacing:.5 }}>Ubicación</p>
                <input className="inp" placeholder="Ciudad, región o zona" value={f.location} maxLength={100} onChange={e=>upd("location",e.target.value)}/>
              </div>
              ...
              <button className="btn-red" onClick={submit} disabled={loading||!f.title||(f.rate_type!=="convenir"&&!f.price)}
                style={{ marginTop:8,opacity:(!f.title||(f.rate_type!=="convenir"&&!f.price)||loading)?.5:1,padding:"15px",fontSize:16 }}>
                {loading?<Spin/>:"Publicar arriendo"}
              </button>
            </div>
          )}
...
function MobileLayout({ tab, setTab, session, profile, selected, setSelected, chatListing, setChatListing, openChat, logout, region, setRegion, guestMode, guestSearch, searchCat, setSearchCat, onGuestLogin, onGuestRegister }) {
  ...
  const goRentals  = ()=>{ setSelected(null); setChatListing(null); setTab("arriendos"); };
  ...
  const onPublishRental  = ()=> session ? (setPublishKind("arriendo"), setShowPublish(true)) : onGuestRegister();
  ...
  const MENU_NAV = [
    ...
    { key:"nav_rentals",  action:goRentals },
  ];
  ...
  return (
    <div style={{ background:BG, minHeight:"100dvh", color:TEXT }}>
      ...
      <div style={{ paddingTop: `calc(64px + env(safe-area-inset-top))`, paddingBottom:90, ...((tab==="messages"||tab==="profile") ? {} : { paddingLeft:14, paddingRight:14 }) }}>
        ...
        {tab==="arriendos" &&<RentalsPage user={session?.user||null} onSelect={setSelected} region={region} onPublish={onPublishRental}/>}
  ...
  useEffect(()=>{
    ...
    if (selected?.title) {
      ...
      const VISTAS = {
        ...
        arriendos:["Arriendo de maquinaria y equipos",
                   "Arrienda maquinaria, equipos, bodegas, espacios o vehículos con conductor. Contacto directo con quien lo ofrece, sin comisiones."],
    ...
    if (showAuthMode === "landing") return (
      <LangCtx.Provider value={{ lang, setLang, t }}>
        <LandingPage
          ...
          onEnterRentals={()=>{ setGuestMode(true); setTab("arriendos"); }}
        />
      </LangCtx.Provider>
    );
    return (
